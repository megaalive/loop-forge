...
- **Sound panel jujur per instrument (progressive disclosure).** Helper `applySoundVisibility()` menyembunyikan kontrol sesuai `tr.type` yang benar-benar dibaca `hit()`: **Tune** hanya untuk `synth`/`tom`/`sample`; blok **Osc A/B, mix, FM, filter, ADSR** hanya untuk `synth`; blok **file/mode/waveform/slice pads** hanya untuk `sample`. Name/Instrument/**Hear this sound** selalu tampil. Mengganti Instrument langsung menyinkronkan **seluruh** UI yang bergantung pada `tr.type` (row ♪/pitch toggle, inline piano, hint **load sample**, field Step panel, dan tombol **Notes** global) lewat satu `renderAll()` — tanpa reload dan tanpa sync path kedua. Hint **load sample** untuk sample track tanpa buffer tetap ada, dan gesture file-picker tidak rusak.
...
- **Seed dipisahkan dari blok Local text** menjadi baris **Generation seed** sendiri di More & tools. Seed hanya dibaca oleh operasi Generator: `runGen()` (local text), `generateAllParts()` (A–D), dan `variationToNext()` (Variation) — **tidak** oleh Track tools. Field seed tetap tersedia di Native (dan disembunyikan di LLM) dengan **hint state-aware yang eksplisit** menyebut operasi mana yang memakainya, sehingga field yang terlihat tidak pernah diam-diam diabaikan oleh CTA Generate.
...
- **Fill gaps adalah Track tool yang berdiri sendiri.** Sebelumnya `fillGaps()` mengambil RNG seed dari `genSeed` + `nativePrompt()`, sehingga tool ini diam-diam ikut berubah kalau user mengubah Local text atau Generation seed — padahal copy UI menyatakan Local text hanya dipakai Generate, dan `genSeed` bahkan tersembunyi di mode LLM (melanggar *"no hidden control may affect an available action"*). Sekarang seed Fill gaps murni dari konteks track lokal: `seeded(hash32('fill|'+currentPattern+'|'+selectedTrack))` — deterministik, tanpa `Math.random()` global, dan tidak lagi membaca `genSeed`, `nativePrompt()`, `genTarget`, `genre`/`density`, maupun engine/mode/scope. Karakter musik (probabilitas `.28`/`.34`, ghost velocity, `p=.5`, shift ±10, perilaku lock/checkpoint) tidak berubah. Hint Seed juga tidak lagi menyebut Fill gaps.
